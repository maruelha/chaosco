...
### Retail Requirements Tracker (own vertical — new module pattern, built 2026-07-04)
Spec: `retail-tracker-handoff.md`. SQL in `app/db_retail_tracker.py`, routes in Blueprint `app/web_retail_tracker.py` — deliberately NOT in database.py/web.py. "Tested" is derived live from `retail` statuses; never stored.
- `tracker_countries` — id, name (UNIQUE, join key vs retail.country), excel_header, active, sort_order. The Excel's "18 = all" derives from active rows here.
- `retail_requirements` — id, area (sales/return/payment_general), scenario_label, name, excel_test_ref (Excel GKP…MU01 id, display only), test_name (name-after-underscore, resolution key), test_case_id (resolved dashboard id, NULL = unresolved; manual resolutions survive re-import), sale_test_ref (informational), required_raw, required_dtc, all_countries, comment, excel_row, created_at. UNIQUE(area, excel_row).
- `country_payment_methods` — tab-4 matrix: country (full name, mapped from tab-4 2-letter codes; AU=Austria, BU=Bulgaria; Cyprus appears only here and is NOT in tracker_countries), method_name, category (card/voucher/NULL=needs manual pick; manual picks survive re-import), active, comment (odd Excel cell texts), excel_row. UNIQUE(country, method_name).
- `requirement_country_targets` — named specific-country targets (requirement_id, country; UNIQUE pair). From comments that are pure country lists ("UK, IE" → United Kingdom + Ireland — currently the only case). When present, counting requires EVERY target country, not any-N. Replaced on re-import.
- `tracker_tab4_tests` — the four fixed tab-4 tests (test_kind PK: sale_card/sale_voucher/return_card/return_voucher, excel_test_ref, test_name, test_case_id). Parsed from tab-4 row 2, resolved by name like requirements.
- `cpm_checks` — manual per-method confirmations (cpm_id, test_kind, UNIQUE pair). Presence = "this method WAS covered in that test kind's passed run" — the live pass can't know which methods a run exercised. done = all applicable kinds checked. `country_payment_methods.user_comment` = user-authored comment (importer never touches it).
- `tracker_missing_tests` — user-authored red alarm list on the board (text, created_at).
- `tested_overrides` — requirement_id/cpm_id, test_kind, country, reason (NOT NULL), created_at. The ONLY stored tested-state (step 5).
...
| **Retail Requirements Tracker — Board** | `/retail-tracker/board` | THE tracker view (dashboard card primary button). Two collapsible sections (Tracking Sales blue, Tracking Return purple — NO payment area: that tab was duplicative, its 2 unique rows folded into Return) + tab-4 matrix (slate, collapsed). Rows sorted by test case; columns: Test case, Test name, Scenario, Requirement, Required, Progress — countries behind per-row "countries ▸" expand (chips: green=passed, amber=executed, gray=none, red ring=named target). Stat cards on top. Red **"Tests missing"** alarm list at bottom (`tracker_missing_tests` table, add/delete, routes `/retail-tracker/missing/add`, `/missing/<id>/delete`). |
| **Retail Requirements Tracker — Payment methods** | `/retail-tracker/payment-methods` | Tab-4 management: four fixed tests as cards, per (country × method) row with AJAX check-off per applicable test kind ("● test passed" hint = checking possible), editable comment, inline category select, filters (country multi-select, category, method text). done = all applicable kinds manually checked; "ready" = tests passed, checks pending. |
| **Retail Requirements Tracker — Import & admin** | `/retail-tracker/` | Steps 1–3 admin: one-time import of all 4 tabs (idempotent; manual picks survive re-runs). Tabs 1–3 → `retail_requirements` with name-based resolution (manual pick for unresolved); "1 or 5" imports as 5 (user decision). Tab 4 → `country_payment_methods` + four fixed tests; missing X marks get an inline Card/Voucher picker. Dashboard card badge = unresolved count. |
...
| `app/db_retail_tracker.py` | Retail Requirements Tracker — schema + all its SQL (own module, v2 pattern) |
| `app/retail_tracker_importer.py` | One-time import of the tracking Excel (all 4 tabs) + name resolution |
| `app/retail_tracker_counting.py` | Tracker counting service — pure functions over statuses + overrides; `compute_from_db(conn)` assembles inputs. Nothing stored: 'Passed' derives live, named targets need every listed country. |
| `app/web_retail_tracker.py` | Retail Requirements Tracker routes (Flask Blueprint `/retail-tracker`) |
| `tests/test_retail_tracker_importer.py` | pytest suite for tracker parsing/resolution (first tests in the project) |
